...
pub(super) struct Light2dOccluderTextures(pub(super) HashMap<RetainedViewEntity, CachedTexture>);

/// [`CachedTexture`]s for 2d lights.
#[derive(Resource, Default)]
pub(super) struct Light2dTextures(pub(super) HashMap<RetainedViewEntity, CachedTexture>);

/// Prepare scaled [`CachedTexture`]s and insert into [`Light2dOccluderTextures`].
pub(super) fn prepare_light_2d_occluder_texture(
    views: Query<(&ViewTarget, &ExtractedView)>,
    mut textures: ResMut<Light2dOccluderTextures>,
